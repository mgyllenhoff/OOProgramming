...
Player::Player(string name, bool isHuman)
  : name(name), isHuman(isHuman), chips(50) {}
...
void PokerGame::bettingRound() {
  ...
  for (Player &p : players) {
    if (p.isHuman) {
      ...
      if (bet > p.chips) {
        cout << "You don't have enough chips. Betting your remaining " << 
            p.chips << " chips" << endl;
        bet = p.chips;
      } else if (bet < 0) {
...
// Compares two hands (PokerHand objects)
